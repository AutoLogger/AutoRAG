...
- **`= 1`** for one-shot calls on a *bigger* model. Ollama pre-reserves all `NUM_PARALLEL` slots' KV cache at the configured `num_ctx`, so 4 idle slots steal VRAM that the bigger model needs. With `NUM_PARALLEL=1` on a 24 GB GPU you can run `qwen2.5:14b-q8_0` (~15 GB) at the agent's `num_ctx=8192` (~1.5 GB KV) with full GPU offload and headroom; a single-stream `num_ctx=16384` (~3 GB KV) still fits, 32K KV pushes some layers onto CPU. Verify with `ollama ps` after a load. With all stages now sharing one `num_ctx` and the model staying warm, `NUM_PARALLEL>=4` is unambiguously beneficial — no per-stage reload cost to trade off against.
...
- Per-slot KV-cache sizing (f16): every stage uses one `num_ctx=8192` to fit 4 slots × KV + ~9 GB model in a 24 GB budget. The uniformity is load-bearing — Ollama reloads on any `num_ctx` change, so a constant size is what keeps the model warm across stages (see the agent-section residency note). `num_ctx_l1` stays overridable for very long audio (raise to 16384, costs one Stage 2→3a reload). Conservative enough that bumping the LLM to `qwen2.5:32b-q4_K_M` typically just needs `NUM_PARALLEL=1` and no other changes.
