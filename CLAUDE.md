...
Python 3.12 project managed with `uv`. Distribution is **GitHub-hosted, not PyPI** — consumers pin to a git tag (`pip install "autorag[...] @ git+https://github.com/AutoLogger/AutoRAG@v0.8.0`). User-facing documentation is in `docs/` (Sphinx); keep this file in sync with it.
...
Total LLM calls per clip ≈ `2 + N1_long + N1_yes + N1 + N2_total` (~20 for a 7-minute clip). Final shape: `{"topics": [L0]}` with `L0.children = [L1...]`, each `L1.children = [L2...]` or `[]`. Boundary calls receive a time-bucketed transcript view (`blocks.format_blocks`, 30s windows by default — tunable via the `boundary_block_seconds` kwarg / `--boundary-block-seconds` flag; one `MM:SS-MM:SS Speaker K: <words>` line per turn instead of one timestamped line per word, keeping boundary prompts compact) and emit `{s, e}` as `MM:SS` strings copied from those range markers; `agent._parse_ts` parses them back to float seconds before tiling (the LLM never does arithmetic). The K summary calls share an identical prompt prefix so Ollama's per-slot prefix cache pays once. All five stages use one `num_ctx` (default 8192) and `keep_alive="5m"`, so the LLM stays resident in VRAM for the whole run instead of cold-reloading per stage (Ollama reloads on any `num_ctx` change); `_build_tree` issues one throwaway `keep_alive=0` call in a `finally` after Stage 5 (or on error) to evict it so it doesn't squat VRAM during the downstream embed/viz step — the LLM analogue of the whisper/pyannote CPU-offload idiom. `num_ctx_l1` stays overridable: very long audio (≈1 hr+) can truncate the L1 transcript at 8192, so raising it back to 16384 trades one Stage 2→3a reload for fidelity. Independent retry: a bad boundary call can be replayed without redoing summaries. **Why split boundaries from summaries:** combined "find sections AND summarize" calls caused earlier LLM versions to bleed section content across boundaries or conflate distinct topics; two focused prompts are more reliable and each stage can be retried independently.

Default LLM model: `gemma4:latest` (8B Q4_K_M, ~9.6 GB; a `thinking`-capable model). Override via `--llm-model` (CLI) or `llm_model=` (SDK). All five stages do mechanical JSON extraction, so the agent sets `reasoning=False` by default — the same determinism/latency rationale as `temperature=0.0` — which sends `think: false` to Ollama and suppresses gemma4's chain-of-thought preamble (a structured-output parse hazard and pure latency tax here). `reasoning` is an overridable kwarg on `build_topic_runnable` / `build_agent` / `AutoRAG.generate_topics` (default `False`); pass `reasoning=True` to A/B the quality↔latency trade-off (the agent-lab `gemma4-thinking` design) or with a non-thinking model where it's a no-op. On non-thinking models `reasoning=False` is harmless.
...
- `autorag transcribe SOURCE [--title -t] [--whisper-model -w] [--language -l] [--persist/--no-persist] [--db]` — Whisper + diarization → `WordSpan` list. With `--persist` (default) writes to SQLite. `--language` defaults to `en`; pass `--language ""` to fall back to Whisper auto-detect (CLI maps `""` → `language=None`, the SDK/agent auto-detect opt-in — `AutoRAG.transcribe` / `agent.transcribe_audio` / `build_agent` default `language="en"` too).
- `autorag generate-topics SOURCE [--provider -p] [--llm-model -m] [--language -l] [--num-ctx-l1] [--num-ctx-fanout] [--max-concurrency] [--min-subdivide-duration-s] [--reasoning/--no-reasoning] [--boundary-block-seconds] [--transcription -T] [--persist/--no-persist]` — full audio→topics pipeline. Accepts a pre-computed `WordSpan` JSON via `-T` to skip Whisper. The six LLM tuning flags forward 1:1 to `AutoRAG.generate_topics` (same names/defaults: `num_ctx_l1`/`num_ctx_fanout`=8192, `max_concurrency`=4, `min_subdivide_duration_s`=120.0, `reasoning`=False, `boundary_block_seconds`=30); `ollama_base_url` stays env-only (`AUTORAG_OLLAMA_BASE_URL`). Persisted topic JSON goes to stdout; a timing breakdown (`whisper`, `agent`, `cli_store_words`, `cli_finalize`, `cli_embed`) goes to stderr.
...
**Devcontainer defaults.** `.devcontainer/start-ollama.sh` exports a tuned set before `ollama serve` (each `:=`-overridable, so an externally-set value wins): `OLLAMA_FLASH_ATTENTION=1`, `OLLAMA_KV_CACHE_TYPE=q8_0`, `OLLAMA_NUM_PARALLEL=4`, `OLLAMA_MAX_LOADED_MODELS=1`. These are server-side only — the Python agent still controls `num_ctx` / `keep_alive` per request. Flash attention + a q8_0 KV cache roughly halve per-slot KV VRAM at near-lossless quality and cut attention memory bandwidth, so the 4 agent slots stay concurrent *and* each call is faster.
...
- **`= 1`** for one-shot calls on a *bigger* model. Ollama pre-reserves all `NUM_PARALLEL` slots' KV cache at the configured `num_ctx`, so 4 idle slots steal VRAM that the bigger model needs. The default `gemma4:latest` (~9.6 GB) lands at ~11 GB total with the 4 slots at `num_ctx=8192` and `q8_0` KV — comfortable on a 24 GB GPU (in-container ~22 GiB usable) with full GPU offload. The bigger-model case is now `gemma4:26b` (the 25.8B sibling, ~17 GB): drop to `NUM_PARALLEL=1` so its single stream gets the freed slot KV. Verify with `ollama ps` after a load. With all stages sharing one `num_ctx` and the model staying warm, `NUM_PARALLEL>=4` is unambiguously beneficial for the small default — no per-stage reload cost to trade off against.

Other settings:

- **Do NOT** combine `OLLAMA_FLASH_ATTENTION=1` with `OLLAMA_MULTIUSER_CACHE=true` and concurrent slots — triggers `GGML_ASSERT(is_full && "seq_cp() is only supported for full KV buffers")`. Since the devcontainer now ships `FLASH_ATTENTION=1` with `NUM_PARALLEL=4` (concurrent slots) by default, `MULTIUSER_CACHE` must stay unset — `start-ollama.sh` deliberately omits it. The per-slot prefix cache (what the K identical summary prompts benefit from) works without it.
- Per-slot KV-cache sizing: with the default `q8_0` KV cache, each slot's KV is ~half its f16 size, so the 4 slots × KV + the ~9.6 GB `gemma4:latest` weights sit at ~11 GB total — a 24 GB budget now has wide headroom (the old qwen2.5:14b-q8_0 at ~15 GB left it tighter). Every stage still uses one `num_ctx=8192` — the uniformity is load-bearing: Ollama reloads on any `num_ctx` change, so a constant size is what keeps the model warm across stages (see the agent-section residency note). `num_ctx_l1` stays overridable for very long audio (raise to 16384, costs one Stage 2→3a reload); with gemma4's small footprint that extra KV is essentially free VRAM-wise — the cost is the one reload, not memory. Bumping the LLM to `gemma4:26b` typically just needs `NUM_PARALLEL=1` and no other changes. **Validation caveat:** the agent-lab LEDGER's gemma4 rows ran under Ollama's *default* server env (FA default-on, f16 KV), not this tuned `q8_0`-KV + explicit `FLASH_ATTENTION=1` + `NUM_PARALLEL=4` combo; Gemma-family interleaved sliding-window attention is historically sensitive with flash attention, so re-run `bench.py` to confirm gemma4 quality holds under the tuned settings before relying on it.
...
Build backend: `uv_build`. Releases: bump `__version__` in `src/autorag/__init__.py` and `version` in `pyproject.toml`, `uv lock`, commit, `git tag v0.x.0 && git push --tags`. Current version: `0.8.0`.
